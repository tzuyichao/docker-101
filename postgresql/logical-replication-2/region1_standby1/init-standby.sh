...
# 等 primary 起來
until pg_isready -h region1_primary -p 5432 -U postgres >/dev/null 2>&1; do
  echo "waiting for region1_primary ..."
  sleep 1
done
...
if [ -z "$(ls -A /var/lib/postgresql/data 2>/dev/null || true)" ]; then
  ...
  export PGPASSWORD=repl_r1_phy_pwd
  gosu postgres pg_basebackup \
    -h region1_primary -p 5432 -U repl_r1_phy \
    -D /var/lib/postgresql/data \
    -Fp -Xs -P -R
fi

echo "starting region1 standby..."
